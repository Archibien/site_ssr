...
const { data } = await useAsyncData<Response>(
  `typologies:${typology.slug}`,
  () =>
    $fetch('/api/typologies', {
      query: { types: typology.actions.join(','), category: typology.category },
    }),
  { server: true }
)
</script>
...
<template>
  <Hero
    ...
    <div class="space-y-1">
      <div class="flex items-center space-x-2 text-sm text-white mb-m">
        ...
        <span class="font-semibold cursor-pointer">{{ typology.name }}</span>
      </div>
  ...
  <section class="app-container my-xl lg:my-xxl">
    <div class="mb-l md:mb-xl">
      <h3
        class="font-semibold text-darkblue text-title-m max-w-title text-center mx-auto mb-min md:mb-xs">
        {{ typology.title_page }} avec un architecte, les grandes étapes
      </h3>
      <p class="text-gray-600 max-w-title mx-auto text-center">
        Pour votre projet {{ typologyPrefix }} {{ typologyName }}, vous pouvez travailler avec un
        cabinet d’architectes pour vous accompagner sur la totalité d’un projet, ou seulement pour
        quelques étapes : conception, obtention du Permis de Construire, consultation des
        entreprises et/ou gestion du chantier.
      </p>
    </div>
  ...
  <section v-if="data?.references && data.references.length > 0" class="mt-xl lg:mt-maximum mb-xl">
  ...
  <section v-if="data?.agencies && data.agencies.length > 0" class="mt-xl lg:mt-maximum mb-xl">
    <div class="app-container mb-m sm:mb-l">
      <h2 class="text-darkblue font-semibold text-title-l mb-m md:mb-l">Besoin d'inspiration ?</h2>
  ...
  <SectionsContact
    ...
    :title="`Vous souhaitez réaliser votre ${typologyName} avec Archibien ?`" />
  ...
  <SectionsCities :typologies="true" />
</template>
